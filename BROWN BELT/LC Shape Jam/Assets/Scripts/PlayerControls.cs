...
public class PlayerControls : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((Input.GetKey("space") || Input.GetKey("z")) && (Time.time > nextFire)) 
        {
            ...
            if(currentLevel>= 3)
            {
                Vector3 topRightOffset = new Vector3(0.1f, 0.2f, 0);
                Vector3 topLeftOffset = new Vector3(-0.1f, 0.2f, 0);
                Instantiate(projectile, transform.position + topRightOffset, transform.rotation);
                Instantiate(projectile, transform.position + topLeftOffset, transform.rotation);
            }
